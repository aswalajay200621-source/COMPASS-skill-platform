 ...
 * 
 * Includes clean extension point interface for optional future AI layer.
 */
...
/**
 * Finds the best-matching faculty mentor for a project by comparing project skills against registered faculty skills.
 */
function findBestMentorForProject(projectRequiredSkills, faculties = []) {
  if (!faculties || faculties.length === 0 || !projectRequiredSkills || projectRequiredSkills.length === 0) {
    return null;
  }

  const matches = faculties.map(faculty => {
    const facultySkills = Array.isArray(faculty.skills)
      ? faculty.skills
      : JSON.parse(faculty.skills || '[]');
...
/**
 * Reserved Extension Point for AI Layer (v2):
 * Accepts a skill gap and student context, and returns a human-friendly encouraging roadmap explanation.
 * For v1, this returns a deterministic fallback response.
 */
async function explainSkillGapAI(missingSkills, studentName) {
  return {
    isAiGenerated: false,
